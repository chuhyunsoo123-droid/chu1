...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (curl_errno($ch)) { echo "⚠️ 서버 연결 에러: " . curl_error($ch); }
    else {
        $decoded = json_decode($response, true);
        if (isset($decoded['textResponse'])) { echo $decoded['textResponse']; }
        else { echo "⚠️ 답변을 가져오지 못했습니다."; }
    }
    curl_close($ch);
    exit;
}
...
<head>
    ...
    <style>
        ...
        header {
            ...
            background: #fff;
            z-index: 10;
            /* 좌우 테두리는 main-container가 담당하므로 제거 */
        }
